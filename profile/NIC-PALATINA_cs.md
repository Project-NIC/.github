...
naakumulované srážky váží místo překlápění — doplněný Modbus meteo senzory:
teplota/vlhkost, vítr, tlak, osvit, UV a půda. (Blesky jsou teď vlastní front, **Tesla**.)

Je to univerzální jádro NIC node plus meteo front (knihovny `nw-*`) a glue, které mluví
RS-485 / Modbus. V platformě navíc dělá **hub** (typ node `basic`), na který se věší
ostatní fronty.
